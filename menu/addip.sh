...
adminip=$( curl -sS https://raw.githubusercontent.com/bestmpc/access/main/accessmenu | grep $MYIP )
...
setadmin() {
#isi link git
linkinstall="https://raw.githubusercontent.com/bestmpc/allow/main"
rm -rf /etc/adminip/
mkdir -p /etc/adminip
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/bestmpc/access/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
rm -rf /root/allow
git config --global user.email "Argantara627.psa@gmail.com"
git config --global user.name "bestmpc"
git clone https://github.com/bestmpc/access.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/bestmpc/access.git
git push -f https://${tokengit}@github.com/bestmpc/access.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/bestmpc/vpsme/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
...
echo "  pastikan domain sudah di pointing di CF sebelum install  "
echo -e "\033[0;34m----------------------------------------\033[0m"
rm -rf /root/allow
rm -rf /root/data
...
rm -rf /root/data
rm -rf /root/ipvps.conf
git config --global user.email "Argantara627@gmail.com"
git config --global user.name "bestmpc"
git clone https://github.com/bestmpc/access.git
mkdir /root/allow
cd /root/allow/
