...
echo -e "$NC"
rm -f /usr/bin/user
export username=$( curl -sS https://raw.githubusercontent.com/${GitUser}/access/main/ipvps.conf | grep $MYIP | awk '{print $2}' )
echo "$username" > /usr/bin/user

# // ORDER ID
rm -f /usr/bin/ver
export user=$( curl -sS https://raw.githubusercontent.com/${GitUser}/access/main/ipvps.conf | grep $MYIP | awk '{print $3}' )
echo "$user" > /usr/bin/ver

# // VALIDITY
rm -f /usr/bin/e
export valid=$( curl -sS https://raw.githubusercontent.com/${GitUser}/access/main/ipvps.conf | grep $MYIP | awk '{print $4}' )
...
echo -e "\e[0;34m-----------------------------------------------------------\e[0m"
echo -e ""
echo -e "${green}JIKA NOT RUNNING, PLEASE REPORT TO ADMIN FOR FIX$NC"
echo ""
