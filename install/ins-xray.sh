...
VALIDITY () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/${GitUser}/access/main/ipvps.conf | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/${GitUser}/access/main/ipvps.conf | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
mkdir /root/.acme.sh
curl https://raw.githubusercontent.com/bestmpc/vpsme/main/acme.sh -o /root/.acme.sh/acme.sh
